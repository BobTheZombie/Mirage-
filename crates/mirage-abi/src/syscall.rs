//! Stable syscall ABI shared by Mirage kernel entry and userspace wrappers.
//!
//! # Calling convention
//!
//! On x86_64, userspace places the syscall number in `rax` and up to six
//! integer/pointer arguments in `rdi`, `rsi`, `rdx`, `r10`, `r8`, and `r9`,
//! then executes `syscall`. The kernel returns an `isize`-sized value in
//! `rax`. Non-negative values are successful results. Negative values at the
//! userspace wrapper boundary are libc-style `-errno` values. Kernel trap
//! handlers may also use [`MIRAGE_SYSCALL_ERROR_BIT`] in an internal raw `u64`
//! return to distinguish encoded [`SyscallErrorCode`] values before wrappers
//! translate them.
//!
//! Syscall numbers are append-only ABI. Existing assignments must never be
//! reused for a different operation. Mirage intentionally keeps Linux-like
//! aliases for selected POSIX-facing calls (`openat`, `wait4`, `execve`) while
//! routing policy through Mirage supervisor/capability mechanisms.
...
// Userspace-friendly aliases. Keep these in the shared ABI crate so no_std
// runtime code does not duplicate the kernel's table.
pub const SYS_GETPID: usize = MIRAGE_SYSCALL_GETPID as usize;
pub const SYS_SPAWN: usize = MIRAGE_SYSCALL_SPAWN as usize;
pub const SYS_OPENAT: usize = MIRAGE_SYSCALL_OPENAT as usize;
pub const SYS_CLOSE: usize = MIRAGE_SYSCALL_CLOSE as usize;
pub const SYS_READ: usize = MIRAGE_SYSCALL_READ as usize;
pub const SYS_WRITE: usize = MIRAGE_SYSCALL_WRITE as usize;
pub const SYS_GETDENTS64: usize = MIRAGE_SYSCALL_GETDENTS64 as usize;
pub const SYS_EXECVE: usize = MIRAGE_SYSCALL_EXECVE as usize;
pub const SYS_EXIT: usize = MIRAGE_SYSCALL_EXIT as usize;
pub const SYS_WAIT4: usize = MIRAGE_SYSCALL_WAIT4 as usize;
pub const SYS_WAIT: usize = SYS_WAIT4;
pub const SYS_YIELD: usize = MIRAGE_SYSCALL_YIELD as usize;

// Compatibility aliases for the intentionally Linux-like POSIX-facing names.
pub const MIRAGE_SYSCALL_OPEN_AT: u64 = MIRAGE_SYSCALL_OPENAT;
pub const MIRAGE_SYSCALL_WAIT: u64 = MIRAGE_SYSCALL_WAIT4;
...
mod tests {
    ...
    #[test]
    fn userspace_aliases_match_kernel_abi_constants() {
        assert_eq!(SYS_GETPID as u64, MIRAGE_SYSCALL_GETPID);
        assert_eq!(SYS_SPAWN as u64, MIRAGE_SYSCALL_SPAWN);
        assert_eq!(SYS_OPENAT as u64, MIRAGE_SYSCALL_OPENAT);
        assert_eq!(SYS_CLOSE as u64, MIRAGE_SYSCALL_CLOSE);
        assert_eq!(SYS_READ as u64, MIRAGE_SYSCALL_READ);
        assert_eq!(SYS_WRITE as u64, MIRAGE_SYSCALL_WRITE);
        assert_eq!(SYS_GETDENTS64 as u64, MIRAGE_SYSCALL_GETDENTS64);
        assert_eq!(SYS_EXECVE as u64, MIRAGE_SYSCALL_EXECVE);
        assert_eq!(SYS_EXIT as u64, MIRAGE_SYSCALL_EXIT);
        assert_eq!(SYS_WAIT4 as u64, MIRAGE_SYSCALL_WAIT4);
        assert_eq!(SYS_WAIT, SYS_WAIT4);
        assert_eq!(SYS_YIELD as u64, MIRAGE_SYSCALL_YIELD);
    }

    #[test]
    fn compatibility_aliases_preserve_existing_numbers() {
        assert_eq!(MIRAGE_SYSCALL_OPEN_AT, 16);
        assert_eq!(MIRAGE_SYSCALL_OPENAT, 16);
        assert_eq!(MIRAGE_SYSCALL_EXECVE, 101);
        assert_eq!(MIRAGE_SYSCALL_WAIT, 103);
        assert_eq!(MIRAGE_SYSCALL_WAIT4, 103);
    }
}
