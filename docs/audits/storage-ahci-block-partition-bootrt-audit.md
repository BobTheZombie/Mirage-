# Mirage Storage, AHCI, Block, Partition, and Spider Runtime Audit
...
- Spider Runtime RuntimeVfs now has a simple immutable archive format, manifest parsing, hash verification via CRC32, path lookup, file read, and read-only enforcement.
- Spider-rs launch policy reads `/spider-rt/sbin/spider-rs` from Spider Runtime and asks MTSS/kernel task creation to launch a userspace task instead of calling Spider-rs as a kernel function.
...
- Spider Runtime image production/staging in the ISO is format-defined but still needs the build artifact to be generated from the Spider-rs ELF and installed as a Limine module.
...
- Spider Runtime hash verification is CRC32 only. Signature verification is not implemented and must not be logged as verified.
...
1. Stage a Spider Runtime image as a Limine module whose path or command line contains `spider-rt`.
2. During kernel boot, find the module before normal rootfs mount policy.
3. Validate the Spider Runtime magic, manifest entries, file offsets/sizes, and per-file hash.
4. Mount it as immutable RuntimeVfs at `/spider-rt` and mark Spider Runtime Online only after `/spider-rt/sbin/spider-rs` resolves.
5. If no image exists while Spider-rs is required, mark Spider Runtime Failed, not Skipped.

## Exact plan to launch Spider-rs from Spider Runtime

1. Mount Spider Runtime before normal rootfs selection.
2. Supervisor/userspace-loader path reads `/spider-rt/sbin/spider-rs` from RuntimeVfs.
