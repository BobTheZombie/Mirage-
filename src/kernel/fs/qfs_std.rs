...
pub struct QfsImageReport {
    pub superblock: super::qfs::QfsSuperblock,
...
pub fn mkfs_image<P: AsRef<std::path::Path>>(
    ...
    sector.fill(0);
    super::qfs::serialize_inode_records(&inodes, &mut sector)?;
...
pub fn dump_superblock<P: AsRef<std::path::Path>>(
    path: P,
) -> Result<super::qfs::QfsSuperblock, QfsToolError> {
    let file = std::fs::OpenOptions::new().read(true).open(path)?;
    let device = StdQfsBlockDevice::new(file)?;
    let mut sector = [0u8; QFS_SECTOR_SIZE];
    crate::kernel::device::BlockStorageDevice::read_sectors(&device, 0, &mut sector)?;
    Ok(super::qfs::QfsSuperblock::parse_sector(&sector)?)
}
...
fn open_userspace_qfs<P: AsRef<std::path::Path>>(
    path: P,
    read_only: bool,
) -> Result<super::qfs::QfsFileSystem, QfsToolError> {
    let file = std::fs::OpenOptions::new()
        .read(true)
        .write(!read_only)
        .open(path)?;
    ...
    let fs = super::qfs::QfsFileSystem::new_on_block_device(read_only, device);
    fs.refresh_from_block_device()?;
    Ok(fs)
}

fn image_report(fs: &super::qfs::QfsFileSystem) -> Result<QfsImageReport, QfsToolError> {
    use crate::kernel::fs::vfs::FileSystem;
    ...
    Ok(QfsImageReport {
        superblock: super::qfs::QfsSuperblock {
