//! Shared QFS on-disk format helpers.
//!
//! This module is intentionally small and stable enough for both the in-kernel
//! QFS driver and hosted `qfsprogs` tooling to agree on the exact wire format.
...
/// Reads one QFS sector from a block device.
pub fn read_sector(
...
/// Writes one QFS sector to a block device.
pub fn write_sector(
    device: &dyn BlockStorageDevice,
    sector: u64,
...
/// Reads and parses the sector-zero QFS superblock from a block device.
pub fn read_superblock(device: &dyn BlockStorageDevice) -> Result<QfsSuperblock, FsError> {
    let mut sector = [0u8; QFS_SECTOR_SIZE];
    read_sector(device, QFS_SUPERBLOCK_SECTOR, &mut sector)?;
    parse_superblock(&sector)
}

/// Serializes and writes the sector-zero QFS superblock to a block device.
pub fn write_superblock(
    device: &dyn BlockStorageDevice,
    superblock: &QfsSuperblock,
) -> Result<(), FsError> {
    let mut sector = [0u8; QFS_SECTOR_SIZE];
    serialize_superblock(superblock, &mut sector)?;
    write_sector(device, QFS_SUPERBLOCK_SECTOR, &sector)
}

/// Reads and parses one book header from a block device.
pub fn read_book_header(
    device: &dyn BlockStorageDevice,
    superblock: &QfsSuperblock,
    book_id: u32,
) -> Result<QfsBookHeader, FsError> {
    let mut sector = [0u8; QFS_SECTOR_SIZE];
    read_sector(device, book_start_sector(superblock, book_id), &mut sector)?;
    parse_book_header(&sector)
}

/// Serializes and writes one book header to a block device.
pub fn write_book_header(
    device: &dyn BlockStorageDevice,
    superblock: &QfsSuperblock,
    header: &QfsBookHeader,
) -> Result<(), FsError> {
    let mut sector = [0u8; QFS_SECTOR_SIZE];
    serialize_book_header(header, &mut sector)?;
    write_sector(
        device,
        book_start_sector(superblock, header.book_id),
        &sector,
    )
}

/// Reads the fixed-size inode table sector referenced by the superblock.
pub fn read_inode_table(
    ...
    let sector_number =
        page_location_sector(superblock, superblock.inode_table).ok_or(FsError::Unsupported)?;
    let mut sector = [0u8; QFS_SECTOR_SIZE];
    read_sector(device, sector_number, &mut sector)?;
    parse_inode_records(&sector, inodes)
}

/// Serializes and writes the fixed-size inode table sector referenced by the superblock.
pub fn write_inode_table(
    device: &dyn BlockStorageDevice,
    superblock: &QfsSuperblock,
    ...
    write_sector(device, sector_number, &sector)
}

/// Scans the on-disk journal descriptors and validates their fixed-width headers.
pub fn scan_journal(
    device: &dyn BlockStorageDevice,
    superblock: &QfsSuperblock,
    records: &mut [Option<QfsJournalRecord>; QFS_MAX_JOURNAL_RECORDS],
) -> Result<(usize, QfsTransactionId), FsError> {
    records.fill(None);
    ...
    Ok((slot_count, max_transaction_id))
}

/// Replays all complete, non-aborted transactions from a scanned journal.
pub fn replay_valid_journal_records(
    device: &dyn BlockStorageDevice,
    superblock: &QfsSuperblock,
    ...
    let mut idx = 0usize;
    while idx < min(slot_count, records.len()) {
...
/// Initializes a minimal QFS image containing a root inode and empty metadata tables.
pub fn initialize_image(
    device: &dyn BlockStorageDevice,
    sector_count: u64,
) -> Result<QfsSuperblock, FsError> {
    if device.sector_size() != QFS_SECTOR_SIZE {
        return Err(FsError::Unsupported);
    }
    let book_size = QFS_BOOK_PAGES as u64 * QFS_PAGE_SECTORS as u64;
    if sector_count < QFS_FIRST_BOOK_SECTOR + book_size {
        return Err(FsError::NoSpace);
    }
    let total_books: u32 = ((sector_count - QFS_FIRST_BOOK_SECTOR) / book_size)
        .try_into()
        .map_err(|_| FsError::NoSpace)?;
    let formatted_sectors = QFS_FIRST_BOOK_SECTOR + u64::from(total_books) * book_size;
    if device.sector_count() < formatted_sectors {
        return Err(FsError::NoSpace);
    }
    let reserved_sectors = QFS_SUPERBLOCK_SECTOR
        + 1
        + QFS_BOOK_HEADER_SECTORS
        + QFS_BOOK_INDEX_SECTORS
        + u64::from(QFS_PAGE_SECTORS) * 3;
    let superblock = QfsSuperblock {
        ...
        free_sectors: formatted_sectors.saturating_sub(reserved_sectors),
        ..QfsSuperblock::empty()
    };
    ...
    write_book_header(device, &superblock, &header)?;

    let mut sector = [0u8; QFS_SECTOR_SIZE];
    serialize_book_index_entry(
        &QfsBookIndexEntry {
            chapter_id: 1,
            first_page: 1,
            page_count: 1,
    ...
    write_inode_table(device, &superblock, &inodes)?;
    Ok(superblock)
}

fn map_device_error(error: crate::kernel::device::DeviceError) -> FsError {
    match error {
        crate::kernel::device::DeviceError::NotFound
        | crate::kernel::device::DeviceError::RegistryFull => FsError::NoSpace,
        crate::kernel::device::DeviceError::Busy => FsError::Busy,
        crate::kernel::device::DeviceError::Unsupported
        | crate::kernel::device::DeviceError::BufferTooSmall => FsError::Unsupported,
    }
}

pub const fn encode_inode_kind(kind: InodeKind) -> u8 {
    match kind {
...
pub fn decode_inode_kind(kind: u8) -> InodeKind {
    match kind {
...
pub const fn book_size_sectors(superblock: &QfsSuperblock) -> u64 {
    superblock.page_sectors as u64 * superblock.book_pages as u64
}

pub const fn book_start_sector(superblock: &QfsSuperblock, book_id: u32) -> u64 {
    QFS_FIRST_BOOK_SECTOR + book_id as u64 * book_size_sectors(superblock)
}

pub const fn page_location_sector(
    superblock: &QfsSuperblock,
    location: QfsPageLocation,
) -> Option<u64> {
...
pub fn parse_superblock(sector: &[u8; QFS_SECTOR_SIZE]) -> Result<QfsSuperblock, FsError> {
...
pub fn parse_book_header(sector: &[u8; QFS_SECTOR_SIZE]) -> Result<QfsBookHeader, FsError> {
    let cur = LeCursor::new(sector);
...
pub fn parse_book_index_entries(
...
pub fn parse_book_index_entry(bytes: &[u8], offset: usize) -> Result<QfsBookIndexEntry, FsError> {
...
pub fn serialize_book_index_entry(
...
pub fn serialize_inode_records(
    inodes: &[Option<QfsInodeRecord>; QFS_MAX_INODE_RECORDS],
    sector: &mut [u8; QFS_SECTOR_SIZE],
) -> Result<(), FsError> {
    sector.fill(0);
...
pub fn serialize_journal_record(
...
pub fn journal_slot_count(superblock: &QfsSuperblock) -> usize {
...
pub fn apply_transaction_records(
    device: &dyn BlockStorageDevice,
    journal_start: u64,
    records: &[Option<QfsJournalRecord>; QFS_MAX_JOURNAL_RECORDS],
    transaction_id: QfsTransactionId,
...
pub fn checksum_sector(sector: &[u8; QFS_SECTOR_SIZE]) -> u32 {
...
pub const fn max_u16(left: u16, right: u16) -> u16 {
    if left >= right {
        left
    } else {
        right
...
pub const fn max_u64(left: u64, right: u64) -> u64 {
    if left >= right {
        left
    } else {
        right
